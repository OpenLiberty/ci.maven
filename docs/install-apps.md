...
| --------  | ----------- | -------  |
| appsDirectory | The directory where the application files should be copied. The default value is `dropins` if the application is not configured in the server.xml file. The default value is `apps` if the  application is configured in the server.xml file. If the application is configured and this parameter is set to `dropins`, the `install-apps` goal ends with an error. If the application is not configured and this parameter is set to `apps`, the `install-apps` goal adds the `webApplication` configuration to the target server.xml file. It does not update the source server.xml file.| No | 
