...
| looseConfig | Generate a loose configuration file representing the Maven project package and copy to Liberty server's `apps` or `dropins` directory. The default value is `false`.  This parameter will be ignored if installAppPacakges is set to `dependencies`, or the project packaging type is neither `war` nor `liberty-assembly`.  | No |
