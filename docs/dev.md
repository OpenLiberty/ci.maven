...
N.B. starting in 3.6.1, dev mode invokes `generate-features` if the `generateFeatures` configuration parameter is set to `true` (default is `false`). **This goal will modify the source configuration directory of your application.** See [generate-features](generate-features.md) for details.
