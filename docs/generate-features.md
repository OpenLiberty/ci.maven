...
If you need to disable feature generation, you can start dev mode with the parameter `-DgenerateFeatures=false`. When running dev mode you can toggle the generation of features off and on by typing 'g' and pressing Enter. Normally the goal only scans a class file that has just been updated but you can cause it to rescan all class files by typing 'o' and pressing Enter.
