...
The `copyDependencies` parameter can contain a `location` parameter and multiple `dependency` elements with the following parameters.
...
| location | The optional directory to which the dependency is copied. This can be an absolute path, or a relative path to the ${server.config.dir}. | No |
