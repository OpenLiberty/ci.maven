...
public class GenerateFeaturesMojo extends InstallFeatureSupport {
    ...
    private void generateFeatures() throws PluginExecutionException {
        ...
        try {
            libertyDirPropertyFiles = BasicSupport.getLibertyDirectoryPropertyFiles(installDirectory, userDirectory,
                    serverDirectory);
        } catch (IOException e1) {
            log.debug("Exception reading the server property files", e1);
            log.error(
                    "Error attempting to generate server feature list. Ensure your user account has read permission to the property files in the server installation directory.");
            return;
        }
        ...
        if (missingLibertyFeatures.size() > 0 || featuresToInstall.size() > 0) {
