...
public class StartDebugMojoSupport extends BasicSupport {
    ...
    protected void copyConfigFiles() throws Exception {
        ...
        loadLibertyConfigFromMavenProperties();
    ...
    private void loadLibertyConfigFromMavenProperties() {

        Properties mavenProps = project.getProperties();

        Set<Entry<Object, Object>> entries = mavenProps.entrySet();
        for (Entry<Object, Object> entry : entries) {
            ...
            if (propType != null) {
                ...
                log.info("Processing maven property "+key+" with prefix "+propType.getPrefix()+" and suffix "+suffix+" and value "+value);
                switch (propType) {
    ...
    private void writeJvmOptions(File file, List<String> options, List<String> mavenProperties) throws IOException {
        ...
        try {
            writer = new PrintWriter(file, "UTF-8");
            writer.println(HEADER);
            // write the maven properties first so that they do not take precedence over the options specified with jvmOptions
            for (String option : mavenProperties) {
